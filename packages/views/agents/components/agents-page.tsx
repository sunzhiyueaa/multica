...
export function AgentsPage() {
  ...
  const handleUpdate = async (id: string, data: Record<string, unknown>) => {
    try {
      ...
      qc.invalidateQueries({ queryKey: workspaceKeys.agents(wsId) });
      toast.success(t.agents.page.updated);
    } catch (e) {
      toast.error(e instanceof Error ? e.message : t.agents.page.updated);
      throw e;
    }
  };
  ...
  const handleArchive = async (id: string) => {
    try {
      ...
      qc.invalidateQueries({ queryKey: workspaceKeys.agents(wsId) });
      toast.success(t.agents.page.archivedToast);
    } catch (e) {
      toast.error(e instanceof Error ? e.message : t.agents.page.archivedToast);
    }
  };
  ...
  const handleRestore = async (id: string) => {
    try {
      ...
      qc.invalidateQueries({ queryKey: workspaceKeys.agents(wsId) });
      toast.success(t.agents.page.restored);
    } catch (e) {
      toast.error(e instanceof Error ? e.message : t.agents.page.restored);
    }
  };
          ...
          {filteredAgents.length === 0 ? (
            <div className="flex flex-col items-center justify-center px-4 py-12">
              ...
              <p className="mt-3 text-sm text-muted-foreground">
                {showArchived ? t.agents.page.noArchived : archivedCount > 0 ? t.agents.page.noActive : t.agents.page.noAgents}
              </p>
              {!showArchived && (
                <Button
                  onClick={() => setShowCreate(true)}
      ...
      <ResizablePanel id="detail" minSize="50%">
        ...
        {selected ? (
          ...
          <div className="flex h-full flex-col items-center justify-center text-muted-foreground">
            ...
            <p className="mt-3 text-sm">{t.agents.page.selectToView}</p>
            <Button
              onClick={() => setShowCreate(true)}
              size="xs"
              className="mt-3"
