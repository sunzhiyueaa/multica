...
export function SkillsTab({
  ...
  const handleAdd = async (skillId: string) => {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : t.agents.tabs.skillAddFailed);
    } finally {
      setSaving(false);
      setShowPicker(false);
    }
  ...
  const handleRemove = async (skillId: string) => {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : t.agents.tabs.skillRemoveFailed);
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-sm font-semibold">{t.agents.tabs.skillsTitle}</h3>
          <p className="text-xs text-muted-foreground mt-0.5">
            {t.agents.tabs.skillsDesc}
          </p>
        </div>
        <Button
          variant="outline"
      ...
      {agent.skills.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed py-12">
          ...
          <p className="mt-3 text-sm text-muted-foreground">{t.agents.tabs.skillsNone}</p>
          <p className="mt-1 text-xs text-muted-foreground">
            {t.agents.tabs.skillsNoneDesc}
          </p>
      ...
      {showPicker && (
        <Dialog open onOpenChange={(v) => { if (!v) setShowPicker(false); }}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle className="text-sm">{t.agents.tabs.skillsPickerTitle}</DialogTitle>
              <DialogDescription className="text-xs">
                {t.agents.tabs.skillsPickerDesc}
              </DialogDescription>
            </DialogHeader>
            <div className="max-h-64 overflow-y-auto space-y-1">
              ...
              {availableSkills.length === 0 && (
                <p className="py-6 text-center text-xs text-muted-foreground">
                  {t.agents.tabs.skillsAllAssigned}
                </p>
              )}
            </div>
            <DialogFooter>
