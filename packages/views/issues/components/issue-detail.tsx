  ...
  const handleUpdateField = useCallback(
    (updates: Partial<UpdateIssueRequest>) => {
      ...
      updateIssueMutation.mutate(
        { id, ...updates },
        { onError: () => toast.error(t.issues.toast.updateFailed) },
      );
      ...
      <div className="flex h-full flex-col">
        <PageHeader className="gap-2 bg-background text-sm">
          ...
          <div className="flex items-center gap-1 shrink-0">
            <Tooltip>
              ...
              />
              <TooltipContent side="bottom">{isPinned ? t.common.unpinFromSidebar : t.common.pinToSidebar}</TooltipContent>
            </Tooltip>
            <DropdownMenu>
              <DropdownMenuTrigger
                render={
                ...
                <DropdownMenuSub>
                  ...
                  <DropdownMenuSubContent>
                    ...
                    <DropdownMenuItem onClick={() => {
                      ...
                      {t.issues.dates.tomorrow}
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => {
                      const d = new Date(); d.setDate(d.getDate() + 7);
                      handleUpdateField({ due_date: d.toISOString() });
                    }}>
                      {t.issues.dates.nextWeek}
                    </DropdownMenuItem>
                    {issue.due_date && (
                      <>
                        <DropdownMenuSeparator />
                        <DropdownMenuItem onClick={() => handleUpdateField({ due_date: null })}>
                          {t.issues.dates.clearDate}
                        </DropdownMenuItem>
                      </>
                    )}
                  </DropdownMenuSubContent>
            ...
            <Tooltip>
              ...
              />
              <TooltipContent side="bottom">{t.common.toggleSidebar}</TooltipContent>
            </Tooltip>
          </div>
        </PageHeader>
            ...
            <BacklogAgentHintDialog
              ...
              onMoveToTodo={() => {
                updateIssueMutation.mutate(
                  { id, status: "todo" },
                  { onError: () => toast.error(t.issues.toast.statusUpdateFailed) },
                );
            ...
            <IssuePickerDialog
              ...
              onSelect={(selected) => {
                ...
                toast.success(`${t.issues.toast.setParentSuccess} ${selected.identifier} as parent issue`);
              }}
              t={t}
            />
            ...
            <IssuePickerDialog
              ...
              onSelect={(selected) => {
                updateIssueMutation.mutate(
                  ...
                  { onError: () => toast.error(t.issues.toast.addSubIssueFailed) },
                );
                toast.success(`${t.issues.toast.addSubIssueSuccess} ${selected.identifier} as sub-issue`);
              }}
              t={t}
            />
        ...
        <div className="mx-auto w-full max-w-4xl px-8 py-8">
          ...
          {parentIssue && (
            ...
            >
              <span className="font-medium shrink-0">{t.issues.detail.subIssueOf}</span>
              <StatusIcon status={parentIssue.status} className="h-3.5 w-3.5 shrink-0" />
                    ...
                    <TooltipTrigger
                      render={
                        <button
                          ...
                          aria-label={t.issues.detail.addSubIssue}
                        >
                          <Plus className="h-4 w-4" />
                        </button>
                      }
                    />
                    <TooltipContent side="bottom">{t.issues.detail.addSubIssue}</TooltipContent>
                  </Tooltip>
                </div>
          ...
          <div>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <h2 className="text-base font-semibold">{t.issues.detail.activity}</h2>
              </div>
