  ...
  if (usage.length === 0) {
    return (
      <div className="flex flex-col items-center rounded-lg border border-dashed py-6">
        ...
        <p className="mt-2 text-xs text-muted-foreground">{t.runtimes.usage.noUsage}</p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      {totals.cost > 0 && (
        <div className="rounded-lg border bg-muted/30 px-3 py-2">
          <span className="text-xs text-muted-foreground">
            {t.runtimes.usage.estimatedCost.replace("{days}", String(days))}
          </span>
      ...
      <div className="rounded-lg border">
        <div className="grid grid-cols-[100px_1fr_80px_80px_80px_80px] gap-2 border-b px-3 py-2 text-xs font-medium text-muted-foreground">
          <div>{t.runtimes.usage.date}</div>
          <div>{t.runtimes.usage.model}</div>
          <div className="text-right">{t.runtimes.usage.input}</div>
          <div className="text-right">{t.runtimes.usage.output}</div>
          <div className="text-right">{t.runtimes.usage.cacheRead}</div>
          <div className="text-right">{t.runtimes.usage.cacheWrite}</div>
        </div>
