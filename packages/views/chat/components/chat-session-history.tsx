    ...
    <div className="flex flex-1 flex-col overflow-hidden">
      ...
      <div className="flex items-center gap-2 border-b px-4 py-2.5">
        <Tooltip>
          ...
          <TooltipContent side="bottom">{t.chat.session.back}</TooltipContent>
        </Tooltip>
        <span className="text-sm font-medium">{t.chat.session.historyTitle}</span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {sessions.length === 0 ? (
          <div className="flex flex-col items-center justify-center gap-2 py-12 text-muted-foreground">
            ...
            <span className="text-sm">{t.chat.session.noSessions}</span>
          </div>
        ) : (
          <div>
...
function formatTimeAgo(dateStr: string, t: DashboardDict): string {
  ...
  if (diffMins < 1) return t.chat.session.justNow;
  if (diffMins < 60) return `${diffMins}${t.chat.session.minutesAgo}`;
  if (diffHours < 24) return `${diffHours}${t.chat.session.hoursAgo}`;
  if (diffDays < 7) return `${diffDays}${t.chat.session.daysAgo}`;
