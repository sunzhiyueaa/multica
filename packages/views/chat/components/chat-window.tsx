        ...
        <div className="flex items-center gap-0.5 shrink-0">
          <Tooltip>
            ...
            <TooltipContent side="top">
              {isAtMax ? t.chat.window.restore : t.chat.expand}
            </TooltipContent>
          </Tooltip>
          <Tooltip>
            <TooltipTrigger
...
function AgentDropdown({
  ...
  if (!activeAgent) {
    return <span className="text-xs text-muted-foreground">{t.chat.agent.noAgents}</span>;
  }
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" side="top" className="max-h-80 w-auto max-w-64">
        {mine.length > 0 && (
          <DropdownMenuGroup>
            <DropdownMenuLabel>{t.chat.agent.myAgents}</DropdownMenuLabel>
            {mine.map((agent) => (
              <AgentMenuItem
                key={agent.id}
                agent={agent}
        ...
        {others.length > 0 && (
          <DropdownMenuGroup>
            <DropdownMenuLabel>{t.chat.agent.others}</DropdownMenuLabel>
            {others.map((agent) => (
              <AgentMenuItem
                key={agent.id}
                agent={agent}
...
function useStarterPrompts() {
  const { t } = useLocale();
  return useMemo(() => [
    { icon: "📋", text: t.chat.empty.prompts.listTasks },
    { icon: "📝", text: t.chat.empty.prompts.summarize },
    { icon: "💡", text: t.chat.empty.prompts.planNext },
  ], [t]);
}
...
function EmptyState({
  ...
  const { t } = useLocale();
  const STARTER_PROMPTS = useStarterPrompts();

  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-5 px-6 py-8">
      <div className="text-center space-y-1">
        <h3 className="text-base font-semibold">
          {agentName ? t.chat.empty.welcomeAgent.replace("{agent}", agentName) : t.chat.empty.welcomeDefault}
        </h3>
