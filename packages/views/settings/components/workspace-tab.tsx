...
export function WorkspaceTab() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(t.settings.workspaceSettings.updated);
    } catch (e) {
      toast.error(e instanceof Error ? e.message : t.settings.workspaceSettings.updateFailed);
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleLeaveWorkspace = () => {
    ...
    setConfirmAction({
      ...
      description: t.settings.dangerZone.leaveDescription.replace("{workspace}", workspace.name),
      variant: "destructive",
      onConfirm: async () => {
        setActionId("leave");
        ...
        try {
          ...
          toast.error(e instanceof Error ? e.message : t.settings.dangerZone.leaveFailed);
        } finally {
          setActionId(null);
        }
      },
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : t.settings.dangerZone.deleteFailed);
    } finally {
      setActionId(null);
    }
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="space-y-4">
        <h2 className="text-sm font-semibold">{t.settings.workspaceSettings.sectionTitle}</h2>

        <Card>
          <CardContent className="space-y-3">
            <div>
              <Label className="text-xs text-muted-foreground">{t.settings.workspaceSettings.nameLabel}</Label>
              <Input
                type="text"
                value={name}
            ...
            <div>
              <Label className="text-xs text-muted-foreground">{t.settings.workspaceSettings.descLabel}</Label>
              <Textarea
                value={description}
                ...
                placeholder={t.settings.workspaceSettings.descPlaceholder}
              />
            </div>
            <div>
              <Label className="text-xs text-muted-foreground">{t.settings.workspaceSettings.contextLabel}</Label>
              <Textarea
                value={context}
                ...
                disabled={!canManageWorkspace}
                className="mt-1 resize-none"
                placeholder={t.settings.workspaceSettings.contextPlaceholder}
              />
            </div>
            <div>
              <Label className="text-xs text-muted-foreground">{t.settings.workspaceSettings.slugLabel}</Label>
            ...
            <div className="flex items-center justify-end gap-2 pt-1">
              ...
              >
                ...
                {saving ? t.settings.workspaceSettings.updating : t.settings.workspaceSettings.updateButton}
              </Button>
            </div>
            {!canManageWorkspace && (
              <p className="text-xs text-muted-foreground">
                {t.settings.workspaceSettings.onlyAdminsCanManage}
              </p>
            )}
          </CardContent>
        </Card>
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold">{t.settings.dangerZone.sectionTitle}</h2>
        </div>
        ...
        <Card>
          <CardContent className="space-y-3">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-sm font-medium">{t.settings.dangerZone.leaveTitle}</p>
                <p className="text-xs text-muted-foreground">
                  {isSoleOwner
                    ? isSoleMember
                      ? t.settings.dangerZone.leaveSoleOwnerMember
                      : t.settings.dangerZone.leaveSoleOwner
                    : t.settings.dangerZone.leaveNormal}
                </p>
              </div>
              <Button
                variant="outline"
              ...
              >
                {actionId === "leave" ? t.settings.dangerZone.leaving : t.settings.dangerZone.leaveButton}
              </Button>
            </div>
            ...
            {isOwner && (
              <div className="flex flex-col gap-2 border-t pt-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="text-sm font-medium text-destructive">{t.settings.dangerZone.deleteTitle}</p>
                  <p className="text-xs text-muted-foreground">
                ...
                >
                  {actionId === "delete-workspace" ? t.settings.dangerZone.deleting : t.settings.dangerZone.deleteButton}
                </Button>
              </div>
            )}
          </CardContent>
