          ...
          <CardContent className="space-y-3">
            ...
            <div className="grid gap-3 sm:grid-cols-[1fr_120px_auto]">
              ...
              <Select value={tokenExpiry} onValueChange={(v) => { if (v) setTokenExpiry(v); }}>
                ...
                <SelectContent>
                  <SelectItem value="30">30 {t.settings.tokens.days}</SelectItem>
                  <SelectItem value="90">90 {t.settings.tokens.days}</SelectItem>
                  <SelectItem value="365">{t.settings.tokens.year}</SelectItem>
                  <SelectItem value="never">{t.settings.tokens.neverExpiry}</SelectItem>
                </SelectContent>
              </Select>
              <Button onClick={handleCreateToken} disabled={tokenCreating || !tokenName.trim()}>
                {tokenCreating ? t.settings.tokens.creating : t.settings.tokens.createButton}
              </Button>
            </div>
          </CardContent>
        </Card>
                ...
                <CardContent className="flex items-center gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {tok.token_prefix}... · {t.settings.tokens.created} {new Date(tok.created_at).toLocaleDateString()} · {tok.last_used_at ? `${t.settings.tokens.lastUsed} ${new Date(tok.last_used_at).toLocaleDateString()}` : t.settings.tokens.neverUsed}
                      {tok.expires_at && ` · ${t.settings.tokens.expires} ${new Date(tok.expires_at).toLocaleDateString()}`}
                    </div>
                  </div>
                  <Tooltip>
                    <TooltipTrigger
                      render={
                        <Button
                          ...
                          aria-label={`${t.settings.tokens.revokeButton} ${tok.name}`}
                        >
                          <Trash2 className="h-3.5 w-3.5" />
                        </Button>
                      }
                    />
                    <TooltipContent>{t.settings.tokens.revokeButton}</TooltipContent>
                  </Tooltip>
                </CardContent>
              </Card>
            ))}
