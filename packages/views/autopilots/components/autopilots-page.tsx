...
function formatRelativeDate(date: string, t: DashboardDict): string {
  const diff = Date.now() - new Date(date).getTime();
  const days = Math.floor(diff / (1000 * 60 * 60 * 24));
  if (days < 1) return t.autopilots.relativeDate.today;
  if (days === 1) return `1${t.autopilots.relativeDate.daysAgo}`;
  if (days < 30) return `${days}${t.autopilots.relativeDate.daysAgo}`;
  const months = Math.floor(days / 30);
  return `${months}${t.autopilots.relativeDate.monthsAgo}`;
}

function AutopilotRow({ autopilot, t }: { autopilot: Autopilot; t: DashboardDict }) {
  const { getActorName } = useActorName();
  const wsPaths = useWorkspacePaths();

  const STATUS_CONFIG: Record<string, { label: string; color: string; icon: typeof Zap }> = {
    active: { label: t.autopilots.page.active, color: "text-emerald-500", icon: Play },
    paused: { label: t.autopilots.page.paused, color: "text-amber-500", icon: Pause },
    archived: { label: t.autopilots.page.archived, color: "text-muted-foreground", icon: AlertCircle },
  };

  const EXECUTION_MODE_LABELS: Record<string, string> = {
    create_issue: t.autopilots.executionMode.createIssue,
    run_only: t.autopilots.executionMode.runOnly,
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      try {
        ...
        toast.error(t.autopilots.toast.createdTriggerFailed);
      }
      ...
      toast.success(t.autopilots.toast.created);
    } catch {
      toast.error(t.autopilots.toast.createFailed);
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogTitle>{t.autopilots.page.newAutopilot}</DialogTitle>
        <div className="space-y-5 pt-2">
          ...
          {/* Schedule */}
          <div>
            <label className="text-xs font-medium text-muted-foreground">{t.autopilots.fields.schedule}</label>
            <div className="mt-2">
          ...
          <div className="flex justify-end gap-2 pt-1">
            ...
            <Button size="sm" onClick={handleSubmit} disabled={!title.trim() || !assigneeId || submitting}>
              {submitting ? t.autopilots.actionsLabels.creating : t.autopilots.actionsLabels.create}
            </Button>
          </div>
        </div>
      </DialogContent>
...
export function AutopilotsPage() {
  ...
  const TEMPLATES: AutopilotTemplate[] = [
    {
      title: t.autopilots.templates.dailyNewsDigest,
...
5. Post the digest as a comment on this issue and @mention all workspace members`,
    ...
    {
      title: t.autopilots.templates.prReviewReminder,
...
5. @mention the team to remind them to review`,
    ...
    {
      title: t.autopilots.templates.bugTriage,
...
5. Add a comment explaining your assessment and suggested next steps`,
    ...
    {
      title: t.autopilots.templates.weeklyProgressReport,
...
6. Post the report as a comment on this issue`,
    ...
    {
      title: t.autopilots.templates.dependencyAudit,
...
5. Post a summary report as a comment with actionable items`,
    ...
    {
      title: t.autopilots.templates.documentationCheck,
    ...
    <div className="flex h-full flex-col">
      ...
      <PageHeader className="justify-between px-5">
        ...
        <Button size="sm" variant="outline" onClick={() => openCreate()}>
          ...
          {t.autopilots.page.newAutopilot}
        </Button>
      </PageHeader>
      ...
      <div className="flex-1 overflow-y-auto">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center py-16 px-5">
            ...
            <p className="text-sm text-muted-foreground">{t.autopilots.page.noAutopilots}</p>
            <p className="text-xs text-muted-foreground mt-1 mb-6">
              {t.autopilots.page.noAutopilotsDesc}
            </p>
              ...
              {TEMPLATES.map((templateItem) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => openCreate(templateItem)}
                  >
                    <Icon className="h-5 w-5 shrink-0 text-muted-foreground mt-0.5" />
                    <div className="min-w-0">
                      <div className="text-sm font-medium">{templateItem.title}</div>
                      <div className="text-xs text-muted-foreground mt-0.5 line-clamp-2">{templateItem.summary}</div>
                    </div>
                  </button>
                );
              })}
          ...
          <>
            ...
            <div className="sticky top-0 z-[1] flex h-8 items-center gap-2 border-b bg-muted/30 px-5 text-xs font-medium text-muted-foreground">
              <span className="shrink-0 w-4" />
              <span className="min-w-0 flex-1">{t.autopilots.page.name}</span>
              <span className="w-32 shrink-0">{t.autopilots.page.agent}</span>
              <span className="w-24 text-center shrink-0">{t.autopilots.page.mode}</span>
              <span className="w-20 text-center shrink-0">{t.autopilots.page.status}</span>
              <span className="w-20 text-right shrink-0">{t.autopilots.page.lastRun}</span>
            </div>
            {autopilots.map((autopilot) => (
              <AutopilotRow key={autopilot.id} autopilot={autopilot} t={t} />
            ))}
          </>
        )}
      </div>

      <CreateAutopilotDialog open={createOpen} onOpenChange={setCreateOpen} template={selectedTemplate} t={t} />
    </div>
  );
}