...
import { getTypeLabel } from "./inbox-detail-label";
...
export function InboxPage() {
  ...
  const { toast: inboxToast, menu, page, actions } = t.inbox;
  ...
  const handleSelect = (item: InboxItem) => {
    ...
    if (!item.read) {
      markReadMutation.mutate(item.id, {
        onError: () => toast.error(inboxToast.markReadFailed),
      });
    }
  };
  ...
  const handleArchive = (id: string) => {
    ...
    archiveMutation.mutate(id, {
      onError: () => toast.error(inboxToast.archiveFailed),
    });
  };
  ...
  const handleMarkAllRead = () => {
    markAllReadMutation.mutate(undefined, {
      onError: () => toast.error(inboxToast.markAllReadFailed),
    });
  };
  ...
  const handleArchiveAll = () => {
    ...
    archiveAllMutation.mutate(undefined, {
      onError: () => toast.error(inboxToast.archiveAllFailed),
    });
  };
  ...
  const handleArchiveAllRead = () => {
    ...
    archiveAllReadMutation.mutate(undefined, {
      onError: () => toast.error(inboxToast.archiveAllReadFailed),
    });
  };
  ...
  const handleArchiveCompleted = () => {
    ...
    archiveCompletedMutation.mutate(undefined, {
      onError: () => toast.error(inboxToast.archiveCompletedFailed),
    });
  };
  ...
  const listBody = items.length === 0 ? (
    <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
      ...
      <p className="text-sm">{page.empty}</p>
    </div>
  ) : (
    <div>
      {items.map((item) => (
  ...
  const detailContent = selected?.issue_id ? (
    ...
    <div className="p-6">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        {getTypeLabel(t, selected.type)} · {timeAgo(t, selected.created_at)}
      </p>
      {selected.body && (
